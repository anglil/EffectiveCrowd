...
if __name__ == "__main__":
	...
	testFile = direc+"data_test/test_"+sys.argv[1]+"_new_feature"

	featureFile = direc+"data_featurized/data_only_CS_and_test"
	fileConcat(posFile, negFile, direc+"data_debug/debug")
	fileConcat(direc+"data_debug/debug", testFile, featureFile)
	...
	# for i in range(5):
	# 	posLenRel[i] = getLen(direc+"data_train_CS/train_CS_"+relation[i]+"_pos_comb_new_feature")
	# 	negLenRel[i] = getLen(direc+"data_train_CS/train_CS_"+relation[i]+"_neg_comb_new_feature")

	posLenRelMJ = getLen(posFile)
	negLenRelMJ = getLen(negFile)
	...
	for ii in [1]:#range(4):
		for jj in range(8):
			mydir = direc+"exp_portion/portion"+sys.argv[1]+str(ii)+str(jj)+"/"
			...
			if classifier in ["LR", "perceptron"]:
				...
				if headFile != -1:
					...
					y_test = trainAndTestModel(X_test, y_train, X_train, classifier, negPortion)
				...
				# ------------train and test--------------
				if headFile != -1:
					trainAndTestModelMultiRBinary(headFile, testFile, headFileRes, negPortion)

				for trainingFile in trainingFiles:
					print "training size:", getLen(trainingFile)
					trainAndTestModelMultiRBinary(trainingFile, testFile, trainingFile+trainingFileResSuffix, negPortion)

				if tailFile != -1:
					trainAndTestModelMultiRBinary(tailFile, testFile, tailFileRes, negPortion)
